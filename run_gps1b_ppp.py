#!/usr/bin/env python3
"""
GRACE-FO PPP -- Kalman Filter with per-SV Float-Ambiguity States

Algorithm:
  State vector per epoch: [dX, dY, dZ, clk_r, trop_wet, B_G01, B_G02, ...]
    dX..clk_r (4): ephemeral -- no prior, estimated fresh each epoch
    trop_wet  (1): persistent -- random walk
    B_*      (N): persistent -- float ambiguity per SV, random walk

  Each B_sv absorbs: float(N1-N2 ambiguity) + code-phase bias
  Initialized on first sighting as B = median(L_if - P_if)
  New SVs auto-initialize; stale SVs auto-remove after 10 epochs

  Process noise:
    trop: 1e-6 m2/s  (~1cm/sqrt(hr) random walk)
    B_if: 1e-4 m2/s  (tracks ~1m/4h drift)

  Measurement noise (phase overweighted to suppress code noise):
    Phase: sigma=0.01m  Code: sigma=0.30m
...
def process_day(date_str, nhours=4.0, data_dir='./data', grace_id='C', interval=30.0):
    ...
    # -- KF Persistent State --
    # State layout per epoch: [dX, dY, dZ, clk_r, trop_wet, B_G01, B_G02, ...]
    #   dX..clk_r (4): ephemeral -- no prior, estimated fresh each epoch
    #   trop_wet  (1): persistent -- random walk
    #   B_*      (N): persistent -- random walk, one per active SV
    #
    # Per-SV code bias (sv_bias) is pre-estimated from the first N_BIAS epochs
    # and subtracted from both L_r and P_r. Without this, +/-10m per-SV DCB
    # biases corrupt the KF position directly.
    N_EPH = 4

    # Persistent state: {key: float} for mean, {key: float} for variance
    pstate = {}
    pcov = {}
    ...
    epoch_geo = {}
    sv_p_residuals = defaultdict(list)  # for sv_bias estimation
    ...
    # -- Estimate per-SV code biases from first N_BIAS epochs --
    sv_bias = {}
    ...
    results = []
    sv_seen_ago = {}
    sv_bias_buf = defaultdict(list)  # online sv_bias estimation for new SVs
    NEW_SV_BIAS_BUF = 5
    ...
    for i_ep, gps_sod in enumerate(epochs):
        ...
        # --- Time update: add process noise to persistent states ---
        ...
        # Buffer new SVs: B_if (L_if-P_if) and code bias (P_r)
        for sv in svs_now:
        ...
        # Online sv_bias estimation for newly-risen SVs
        ...
        for sv in list(sv_init.keys()):
            ...
            if len(sv_init[sv]) >= INIT_BUF or (len(sv_init[sv]) >= 1 and i_ep <= 2):
                ...
                pcov[sv] = 0.5  # sigma ~0.7m initial
                del sv_init[sv]

        # Track and remove stale SVs
        ...
        # Initialize trop on first epoch
        ...
        sv_idx = {}
        trop_idx = N_EPH  # trop is always first persistent key
        ...
        # Prior rows for persistent states
        for i, key in enumerate(pkeys):
            h = np.zeros(n_total)
            h[N_EPH + i] = 1.0
            A_rows.append(h)
        ...
        for d in ep_data:
            ...
            # Phase: includes B_if term (sv_bias corrected)
            ...
            # Code: no B_if term (sv_bias corrected)
    ...
    print("\n" + "="*65)
    print(f"  GRACE-FO PPP -- Kalman Filter (per-SV Float Ambiguity)")
    ...
    return {
        ...
        'sv_bias': {},  # KF absorbs sv_bias into B_if; no separate estimate
    }
...
def generate_plots(output, output_dir='./output'):
    ...
    axes[0].set_title(f'GRACE-FO PPP Error vs GNV1B (Kalman Filter) -- {date_str}')
...
def main():
    parser = argparse.ArgumentParser(description='GRACE-FO PPP -- Kalman Filter with Float-Ambiguity')
