...
def process_day(date_str, nhours=4.0, data_dir='./data', grace_id='C', interval=30.0):
    ...
    # Process noise (m2/s)
    Q_TROP = 1e-6
    Q_B    = 2e-5  # Moderate: ~20cm accumulated drift over 30min

    # Measurement noise (flat weights, matching V2.2.0 baseline)
    STD_PHASE = 0.01
    STD_CODE  = 0.30
    ...
    for i_ep, gps_sod in enumerate(epochs):
        ...
        for sv_id, rec in gps1b_raw[gps_sod].items():
            ...
            ep_data.append({
                ...
                'slip': rec.get('slip_L1', False) or rec.get('slip_L2', False),
            })
