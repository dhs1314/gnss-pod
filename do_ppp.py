#!/usr/bin/env python3
"""使用验证过的 PPPProcessor 处理 Apr 29-30 + May 1-2"""
import sys, os, csv, io, ssl, struct, zlib, urllib.request, urllib.error, tarfile, datetime as dt
from pathlib import Path
sys.path.insert(0, 'src')
from ppp import PPPProcessor, ecef_to_blh, ecef_to_enu_matrix
import numpy as np

C=299792458.0; F1,F2=1575.42e6,1227.60e6; MU_E=3.986004418e14; OMEGA_E=7.2921151467e-5
GPS_ORIGIN=dt.datetime(1980,1,6)
ISDC="https://isdc-data.gfz.de/grace-fo/Level-1B/JPL/INSTRUMENT/RL04/{year}/"
GPS_SV=[(1,0.0,0.0,0.0,0.000020,55.0,0.0,5153.6),(3,30.0,30.0,60.0,0.000015,54.8,30.0,5153.6),(5,90.0,90.0,180.0,0.000010,54.9,90.0,5153.6),(7,150.0,150.0,300.0,0.000015,54.7,150.0,5153.6),(8,180.0,180.0,0.0,0.000020,55.0,180.0,5153.6),(10,240.0,240.0,120.0,0.000020,55.0,240.0,5153.6),(11,270.0,270.0,180.0,0.000015,54.9,270.0,5153.6),(13,330.0,330.0,300.0,0.000010,54.7,330.0,5153.6),(15,15.0,15.0,90.0,0.000020,65.0,15.0,5153.6),(17,45.0,45.0,150.0,0.000015,64.8,45.0,5153.6),(18,75.0,75.0,210.0,0.000020,65.0,75.0,5153.6),(19,105.0,105.0,270.0,0.000010,64.9,105.0,5153.6),(20,135.0,135.0,330.0,0.000020,65.1,135.0,5153.6),(21,165.0,165.0,30.0,0.000015,64.7,165.0,5153.6),(22,195.0,195.0,90.0,0.000020,65.0,195.0,5153.6),(23,225.0,225.0,150.0,0.000010,64.8,225.0,5153.6),(24,255.0,255.0,210.0,0.000020,65.0,255.0,5153.6),(25,285.0,285.0,270.0,0.000015,64.9,285.0,5153.6),(27,345.0,345.0,30.0,0.000020,64.7,345.0,5153.6),(29,25.0,25.0,120.0,0.000010,56.0,25.0,5153.6),(30,55.0,55.0,200.0,0.000020,56.1,55.0,5153.6)]
...
def rel_corr(rv,sat,vel):
    r=np.linalg.norm(sat); rho=np.linalg.norm(sat-rv)
    tau=rho/C; dt2=OMEGA_E*tau; cr,sr=np.cos(dt2),np.sin(dt2)
    r2=np.array([rv[0]*cr+rv[1]*sr,-rv[0]*sr+rv[1]*cr,rv[2]])
    rho2=np.linalg.norm(sat-r2); tau2=rho2/C; dt3=OMEGA_E*tau2; cr2,sr2=np.cos(dt3),np.sin(dt3)
    r3=np.array([rv[0]*cr2+rv[1]*sr2,-rv[0]*sr2+rv[1]*cr2,rv[2]])
    rf=np.linalg.norm(sat-r3)
    sagnac=(OMEGA_E/C)*(sat[0]*rv[1]-sat[1]*rv[0])
...
def gen_obs(ref,t0,nh,iv=30.0,seed=42):
    rng=np.random.default_rng(seed); n=int(nh*3600/iv); recs=[]
    for k in range(n):
        ...
        lat,lon,_=ecef_to_blh(gf); Re=ecef_to_enu_matrix(lat,lon)
        dw=0.05+rng.normal(0,0.0002*k)
        for sv in GPS_SV:
            svn=f"G{sv[0]:02d}"
            sat,vel=sp(sv,t); rv=sat-gf; rho=np.linalg.norm(rv)
            if not(20e6<rho<50e6): continue
            e_sat=rv/rho; e_enu=Re @ e_sat
            el=np.arcsin(max(-1.0,min(1.0,e_enu[2]))); az=np.arctan2(e_enu[0],e_enu[1])
            if np.degrees(el)<5.0: continue
            rc=rel_corr(gf,sat,vel); mf=1.0/max(np.sin(el),0.05)
            trop=2.3+0.1*mf+dw*mf
            P=rc+trop+rng.normal(0,0.3); L=rc+trop+rng.normal(0,0.003)
            L_if,P_if=iono(L,L,P,P)
            recs.append({'time':t,'sv':svn,'sat_pos':sat,'sat_vel':vel,
...
def make_svg(dE,dN,dU,n,re,RN,RU,r3,label,W=900,H=520):
    ...
    tmin,tmax=0,times[-1] if times else 1
    ...
    for i,(data,col,lbl) in enumerate(rows):
        ...
        for j in range(0,len(times),max(1,len(times)//40)):
            xi=int(px(times[j]))
            if ML<=xi<ML+pw: svg.append(f'<line x1="{xi}" y1="{ry}" x2="{xi}" y2="{ry+ph}" stroke="#e0e0e0" stroke-width="0.5"/>')
        yz=p0(ry); svg.append(f'<line x1="{ML}" y1="{yz}" x2="{ML+pw}" y2="{yz}" stroke="#999" stroke-width="0.8"/>')
        pts=' '.join(f'{px(times[j]):.1f},{py(data[j],ry):.1f}' for j in range(len(times)) if -ymx<=data[j]<=ymx)
        if pts: svg.append(f'<polyline points="{pts}" fill="none" stroke="{col}" stroke-width="1.2"/>')
        rvals={'E (东西)':re,'N (南北)':RN,'U (垂直)':RU}[lbl]
...
def do_day(y,m,d,nh=4):
    ...
    print(f"  PPP: {len(results)} epochs")
    ...
    csv_path=f"output/pppresult_{ds}_{nh:.0f}h.csv"
    ...
    svg=make_svg(dE,dN,dU,len(results),re,RN,RU,r3,f"{ds} {nh}h")
