...
class BatchOrbitLSQv3:
    ...
    def solve(self, r0_prior, v0_prior,
              arc_wl_fixed=None, osb_wl=None, osb_nl=None):
        """Run GN loop with optional arc-level WL + OSB NL fixing.

        arc_wl_fixed: {sv: N_w} from full-arc MW averaging
        osb_wl, osb_nl: {sv: bias_cyc} CODE satellite biases
        """
        ...
        arc_wl = arc_wl_fixed if arc_wl_fixed else {}
        use_osb = bool(arc_wl) and osb_nl is not None
        ...
        bls = BatchLinearSolver(geo_bs, sigma_phase=self.sigma_phase,
                                 sigma_code=self.sigma_code, robust_reweight=False)
        bls_sol = bls.solve()
        fixed_amb = bls_sol['amb_dict']
        float_phase = bls_sol['rms_phase']
        print(f"  [BS-Am] {len(fixed_amb)} SVs, "
              f"phase={float_phase:.3f}m code={bls_sol['rms_code']:.3f}m")

        # Step 1.5: Arc-level WL + OSB NL fixing
        n_arc_nl = 0
        if arc_wl:
            from src.ambiguity import COEFF_W as CW, LAM_N as LN, F1, F2
            import numpy as _np
            amb_fixed_if = {}
            for sv, N_w in arc_wl.items():
                if sv not in fixed_amb: continue
                B_if = float(fixed_amb[sv])
                b_wl = (osb_wl or {}).get(sv, 0.0)
                b_nl = (osb_nl or {}).get(sv, 0.0) if use_osb else 0.0
                if use_osb:
                    N1_float = (B_if - CW * (N_w + b_wl)) / LN - b_nl
                    N1_int = int(round(N1_float))
                    if abs(N1_float - N1_int) < 0.30:
                        amb_fixed_if[sv] = LN * (N1_int + b_nl) + CW * (N_w + b_wl)
                        n_arc_nl += 1
                else:
                    N1_float = (B_if - CW * N_w) / LN
                    N1_int = int(round(N1_float))
                    if abs(N1_float - N1_int) < 0.30:
                        amb_fixed_if[sv] = LN * N1_int + CW * N_w
                        n_arc_nl += 1
            if amb_fixed_if:
                bls2 = BatchLinearSolver(geo_bs, sigma_phase=self.sigma_phase,
                                         sigma_code=self.sigma_code, robust_reweight=False)
                bls2._amb_fixed = amb_fixed_if
                bls2._sigma_amb_fixed = 0.02
                x_lin2, _, _ = bls2._solve_internal()
                fixed_amb = {sv: float(x_lin2[bls2.I_AMB + bls2.sv_to_idx[sv]])
                           for sv in amb_fixed_if if sv in bls2.sv_to_idx}
                for sv in bls_sol['amb_dict']:
                    if sv not in fixed_amb: fixed_amb[sv] = float(bls_sol['amb_dict'][sv])
                osb_str = "+OSB" if use_osb else ""
                print(f"  [BS-Arc{osb_str}] {len(amb_fixed_if)}/{len(arc_wl)} SVs "
                      f"NL-fixed (sigma=0.02m)")
