...
def compute_epoch_geometry(gps_sod, gps1b_raw, sp3, rcv_pos_ecef):
    ...
    for sv_id, rec in recs.items():
        if 'L_if' not in rec:
            continue
        sat_pos, sat_clk, rho_corr = get_sat_geometry(sp3, sv_id, utc_dt, rcv_pos_ecef)
        if sat_pos is None:
            continue
...
def main():
    ...
    parser.add_argument('--sigma-acc', type=float, default=1e-7,
                        help='Process noise: unmodeled acceleration [m/s^2]')
    ...
    parser.add_argument('--chi2-threshold', type=float, default=10.828,
                        help='Chi-square innovation test threshold (default: 10.828, alpha=0.001)')
    ...
    for i_epoch, gps_sod in enumerate(epochs):
        ...
        ep_data = compute_epoch_geometry(gps_sod, gps1b_raw, sp3, rcv_pos_ecef)
