#!/usr/bin/env python3
"""Minimal PPP for Apr 29 - May 2 using PPPProcessor (no plots)"""
import sys, os, csv, io, ssl, urllib.request, tarfile, datetime as dt
...
def satpos(sv,t):
    ...
    pos=np.array([xp*np.cos(Om)-yp*np.cos(inc_r)*np.sin(Om),
                  xp*np.sin(Om)+yp*np.cos(inc_r)*np.cos(Om),yp*np.sin(inc_r)])
...
def get_gnv1b(y,m,d):
    ...
    if gnv.exists() and gnv.stat().st_size>1000: print(f"  [cache] {ds}"); return str(gnv)
    ...
    try:
        ...
        for m in tar.getmembers():
            if 'GNV1B' in m.name and m.name.endswith('.txt'):
                ...
                if f: fo.write_bytes(f.read()); print(f"  extracted: {fo.name}")
        tar.close()
    except Exception as e:
...
def generate_obs(ref,t0,nh,iv=30.0,seed=42):
    ...
    for k in range(n):
        ...
        for sv in GPS_SV:
            svn=f"G{sv[0]:02d}"
            sat,vel=satpos(sv,t); rv=sat-gf; rho=np.linalg.norm(rv)
...
def do_day(y,m,d,nh=4):
    ...
    csv_path=f"output/ppp_{ds}_{nh:.0f}h.csv"
    ...
    return {'date':ds,'csv':csv_path,'re':re,'rn':RN,'ru':RU,'r3':r3,'n':len(results),'dE':dE,'dN':dN,'dU':dU}
...
# Generate SVG + HTML for each day
...
.day-hdr{font-size:16px;font-weight:bold;color:#1a237e;margin-bottom:12px;display:flex;justify-content:space-between}
...
W,H=900,520; ML,MR,MT,MB,gl=60,20,20,36,8
pw=W-ML-MR; ph=(H-MT-MB-2*gl)//3

def svg_for(r):
    n=r['n']; dE,dN,dU=r['dE'],r['dN'],r['dU']
    ...
    ymx=max(ymx,0.5)
    def px(tv): return ML+(tv-times[0])/(tmax-times[0]+1e-9)*(pw-1)
    ...
    for i,(data,col,lbl) in enumerate(rows):
        ...
        rvals={'E (东西)':r['re'],'N (南北)':r['rn'],'U (垂直)':r['ru']}[lbl]
    ...
    svg.append(f'<text x="{W//2}" y="{H-8}" text-anchor="middle" font-size="12" fill="#333">{r["date"]} 4h | 3D RMS={r["r3"]:.2f} cm ({n} epochs)</text>')
    svg.append('</svg>')
    return '\n'.join(svg)
