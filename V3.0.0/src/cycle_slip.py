...
class TurboEdit:
    ...
    def detect_slip_L1(self, sv, L1, L2, P1, P2):
        """Detect cycle slip, return (has_slip, dN1, dN2, slip_mw).

        Returns:
            has_slip: True if either MW or GF detects a slip
            dN1, dN2: estimated L1/L2 slip [cycles] (0 when unreliable)
            slip_mw: True if MW detected the slip (WL ambiguity changed).
                     GF-only slips have slip_mw=False (WL unchanged).
        """
        mw = mw_combination(L1, L2, P1, P2)
        gf = gf_combination(L1, L2)

        # Run detection and capture method-specific flags
        slip_mw, slip_gf = self._detect_from_values_detailed(sv, mw, gf)
        has_slip = slip_mw or slip_gf
        ...
        if has_slip and sv in self._mw_buf and len(self._mw_buf[sv]) >= 2:
            ...
            if slip_mw:
                # MW jump reliable → estimate both dN1 and dN2
                dN1 = (dmw * lambda_2 - dgf) / (lambda_2 - lambda_1)
                dN2 = dN1 - dmw
                return True, round(dN1), round(dN2), True
            else:
                # GF-only: dN1 ≈ dN2, WL unchanged.  dN1/dN2 estimates are
                # degenerate from dmw≈0 noise — don't report them.
                return True, 0, 0, False

        return has_slip, 0, 0, slip_mw

    def _detect_from_values_detailed(self, sv, mw, gf):
        """Like _detect_from_values but returns (slip_mw, slip_gf) separately."""
        slip_mw = False
        slip_gf = False
        ...
        if len(buf_mw) >= 5:
            mw_median = float(np.median(buf_mw))
            mw_std = max(float(np.std(buf_mw)), 0.25)
            if abs(mw - mw_median) > self.mw_threshold * mw_std:
                slip_mw = True
