...
def main():
    ...
    parser.add_argument('--gravity-model', default='GGM05C',
                        choices=['GGM05C', 'EIGEN-6C4'],
                        help='Gravity field model (GGM05C or EIGEN-6C4)')
    ...
    parser.add_argument('--kinematic', action='store_true', default=False,
                        help='Kinematic batch LSQ: per-epoch ECEF position (no dynamics)')
    ...
    if args.gravity_model == 'EIGEN-6C4':
        gravity_path = dp / 'gravity' / 'EIGEN-6C4_N200.gfc'
        gravity_name = 'EIGEN-6C4'
    else:
        gravity_path = dp / 'gravity' / 'GGM05C.gfc'
        gravity_name = 'GGM05C'
    if gravity_path.exists():
        Cnm, Snm, _, GM_grav, R_grav = read_icgem_gfc(str(gravity_path))
        print(f"  Gravity: {gravity_name} Nmax={GRAVITY_NMAX}")
    else:
    ...
    ekf_config = {
        ...
        'gravity_field': str(gravity_path),  # user-selected gravity model
    }
    ...
    if args.batch_lsq_v2 and _pass1_geometry:
        ...
        _kin = args.kinematic and len(_pass1_r_ecef) == len(_pass1_geometry)
        kin_str = "Kinematic" if _kin else "EKF-orbit"
        print(f"\n-- Framework v3 {kin_str} Batch Solver ({args.hours}h, "
              f"{len(_pass1_geometry)} epochs) --")

        # Load OSB data if available (re-use from EKF config if already loaded)
        _osb_wl = _osb_nl = None
        if args.osb_file:
            from src.batch_lsq import read_code_osb
            _osb_wl, _osb_nl = read_code_osb(args.osb_file, quiet=True)

        # Collect WL-fixed dict from EKF pass 1
        _wl_fixed = ekf._wl_fixed if hasattr(ekf, '_wl_fixed') else {}
        ...
        bls = BatchLinearSolver(_pass1_geometry,
                                 ...
                                 clk1b_data=_clk1b_data,
                                 clk1b_sigma=0.50,
                                 epoch_gps_sod=epochs,
                                 wl_fixed=_wl_fixed,
                                 osb_wl=_osb_wl, osb_nl=_osb_nl,
                                 kinematic=_kin,
                                 rcv_ecef_per_epoch=_pass1_r_ecef if _kin else None,
                                 sigma_pos_prior=0.5,
                                 sigma_pos_smooth=0.05)
        bls_sol = bls.solve()
        ...
        n_nl_fixed = len(bls_sol.get('nl_fixed', {}))

        # Kinematic: compute 3D RMS of corrected positions vs GNV1B
        kin_rms_3d = 0.0
        if _kin and bls_sol.get('r_ecef') is not None:
            kin_dr = []
            for i_ep, gps_sod in enumerate(epochs):
                r_gnv = interpolate_ref(ref_orbit, gps_sod)
                if r_gnv is not None:
                    kin_dr.append(np.linalg.norm(
                        bls_sol['r_ecef'][i_ep] - r_gnv))
            kin_rms_3d = np.sqrt(np.mean([d**2 for d in kin_dr])) if kin_dr else 0.0

        print(f"\n-- Framework v3 {kin_str} Results --")
        print(f"  Geometry: EKF orbit (3D RMS = {rms_float:.3f}m)")
        if _kin:
            imp_3d = (rms_float - kin_rms_3d) / rms_float * 100 if rms_float > 0 else 0
            print(f"  Kinematic 3D RMS: {kin_rms_3d:.3f}m (vs EKF: {imp_3d:+.1f}%)")
        print(f"  Phase RMS:  EKF={ekf_phase_rms:.3f}m  Batch={bch_phase_rms:.3f}m")
        if bls_sol.get('rms_phase_float', 0) > 0:
            imp_osb = (bls_sol['rms_phase_float'] - bch_phase_rms)/bls_sol['rms_phase_float']*100
            print(f"  Phase Float RMS: {bls_sol['rms_phase_float']:.3f}m -> "
                  f"Fixed: {bch_phase_rms:.3f}m (OSB gain: {imp_osb:+.1f}%)")
        if ekf_phase_rms > 0:
            imp = (ekf_phase_rms - bch_phase_rms)/ekf_phase_rms*100
            print(f"  Phase improvement vs EKF: {imp:+.1f}%")
        print(f"  Code RMS:   Batch={bch_code_rms:.3f}m")
        print(f"  Ambs: {len(bls_sol['amb_dict'])} SVs, "
              f"OSB NL-fixed: {n_nl_fixed}")

        out_d = Path('results') / 'batch_v3'
        out_d.mkdir(parents=True, exist_ok=True)
        pickle.dump({
            'rms_ekf_orbit': rms_float,
            'rms_kinematic_orbit': kin_rms_3d if _kin else None,
            'rms_phase_ekf': ekf_phase_rms,
            'rms_phase_batch': bch_phase_rms,
            'rms_phase_float': bls_sol.get('rms_phase_float', 0),
            'rms_code_batch': bch_code_rms,
            'nl_fixed': bls_sol.get('nl_fixed', {}),
            'r_ecef': bls_sol.get('r_ecef'),
            'epochs': epochs,
            'config': vars(args),
        }, open(str(out_d / f'batchv3_{date_str}_{grace_id}_{args.hours}h.pkl'), 'wb'))
        print(f"  Saved: {out_d / f'batchv3_{date_str}_{grace_id}_{args.hours}h.pkl'}")
