...
def main():
    ...
    print(f"GPS1B: {len(gps1b)}, GNV1B: {len(gnv)}, SP3: {len(sp3) if sp3 else 0}")
    
    # 30s采样: 原始5s→30s = 每6个取1个
    all_keys = sorted(gps1b.keys())
    step = 6
    sub_keys = all_keys[::step]
    gps1b_sub = {k: gps1b[k] for k in sub_keys}
    print(f"30s采样: {len(gps1b_sub)} epochs (原始{len(gps1b)}×{1/step})")
    ...
    for strat, sp3_orbit in strategies:
        print(f"\n--- {strat} ---")
        t0 = time.time()
        results = run_ppp('2024-05-02', gps1b_sub, gnv, strategy=strat, ref_orbit=sp3_orbit)
        dt = time.time() - t0
        ...
        out_path = f'{ODIR}/ppp_2024-05-02_{strat}_30s.csv'
        ...
        if nv > 0:
            ...
            print(f"  → 推算全天{len(gps1b)}epochs耗时: {dt/len(results)*len(gps1b)/60:.0f}分钟")
        else:
            print(f"  ALL NaN")
