...
for i, line in enumerate(lines):
    ...
    if '# / TYPES OF OBSERV' in tag:
        try:
            ...
            obs_types = [t.strip() for t in toks if t.strip() and len(t.strip()) <= 3]
            # Check if we need continuation lines (types > 9)
            while len(obs_types) < n_obs and i + 1 < len(lines):
                i += 1
                toks = lines[i][6:60].split()
                obs_types.extend([t.strip() for t in toks if t.strip() and len(t.strip()) <= 3])
        except: pass
