...
class BatchLinearSolver:
    ...
    def __init__(self, epoch_geometry,
                 ...
                 robust_reweight=False, sv_coverage=None):
        """
        Args:
            ...
            robust_reweight: if True, use IRLS with Huber weights (k=2.5sigma)
            sv_coverage: {sv: pct} -- per-SV coverage for adaptive amb prior
        """
        ...
        self.robust_reweight = robust_reweight
        self.sv_coverage = sv_coverage if sv_coverage else {}
    ...
    def _solve_internal(self):
        ...
        # ---- Regularization (rank deficiency + coverage-adaptive amb) ----
        for i in range(self.N_epoch):
            ATA[self.I_ZWD + i, self.I_ZWD + i] += 1e-4
        for i, sv in enumerate(self.sv_list):
            cov_pct = self.sv_coverage.get(sv, 1.0)
            reg_amb = 1e-4 + 1e-2 * (1.0 - cov_pct)**2
            ATA[self.I_AMB + i, self.I_AMB + i] += reg_amb
